...
<!--ER.metaModelLibConstructor.module.xslt -->


<xsl:variable name="erMetaModelLibConstructor" as="function(element(er:entity_model)) as map(xs:string,function(*))">
    <xsl:sequence select="
    function ($model as element(er:entity_model)) as map(xs:string,function(*))
{
let 
$entityType 
